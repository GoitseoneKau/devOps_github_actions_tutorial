...
function changeServiceScope(service) {
  ...
  addSystemLog(
    'INFO',
    service === 'ALL' ? 'edge-cdn' : service,
    `User changed active telemetry dashboard viewing scope to: ${service}`
  );
}
...
function updateMetricsAndUI() {
  ...
  if (uptimeVal) uptimeVal.textContent = finalUptime.toFixed(2);
  if (uptimeBar) uptimeBar.style.width = `${finalUptime}%`;
  if (uptimeBarLabel) uptimeBarLabel.textContent = `${finalUptime.toFixed(2)}%`;
  
  const buildtimeVal = $('metric-buildtime-val');
  if (buildtimeVal) buildtimeVal.textContent = baseline.buildTime.toString();
  ...
  if (baseline.cdnRate === 0) {
    if (cdnVal) cdnVal.textContent = 'N/A';
    if (cdnBar) cdnBar.style.width = '0%';
    if (cdnBarLabel) cdnBarLabel.textContent = 'Not Applicable';
  } else {
    if (cdnVal) cdnVal.textContent = finalCdn.toFixed(1);
    if (cdnBar) cdnBar.style.width = `${finalCdn}%`;
    if (cdnBarLabel) cdnBarLabel.textContent = `${finalCdn.toFixed(1)}%`;
  }

  // Secondary metrics
  const cpuVal = $('metric-cpu-val');
  const cpuBar = $('metric-cpu-bar');
  if (cpuVal) cpuVal.textContent = finalCpu.toFixed(1);
  if (cpuBar) cpuBar.style.width = `${finalCpu}%`;

  const memoryVal = $('metric-memory-val');
  const memoryBar = $('metric-memory-bar');
  if (memoryVal) memoryVal.textContent = finalMem.toFixed(1);
  if (memoryBar) memoryBar.style.width = `${finalMem}%`;

  const netinVal = $('metric-netin-val');
  const netoutVal = $('metric-netout-val');
  if (netinVal) netinVal.textContent = (baseline.netIn * trafficMultiplier + (Math.random() - 0.5) * 5).toFixed(1);
  if (netoutVal) netoutVal.textContent = (baseline.netOut * trafficMultiplier + (Math.random() - 0.5) * 15).toFixed(1);
...
function triggerCpuThresholdBreach(cpu) {
  const now = Date.now();
  if (now - lastThresholdBreachTime < 30000) return; // Limit alerts to once every 30s
...
function addSystemLog(level, service, message) {
  ...
  if (logs.length > 100) logs.pop(); // Cap length
...
function renderLogs() {
  ...
  const searchVal = ($('logs-search-input')).value.toLowerCase() || '';
  const levelVal = ($('logs-level-selector'))?.value || 'ALL';
  const serviceVal = ($('logs-service-selector'))?.value || 'ALL';
  ...
  container.innerHTML = filtered.map(log => {
    ...
    if (log.level === 'SUCCESS') levelClass = 'text-emerald-400 bg-emerald-950/20 px-1 rounded border border-emerald-900/20';
    if (log.level === 'WARN') levelClass = 'text-amber-400 bg-amber-950/20 px-1 rounded border border-amber-900/20';
    if (log.level === 'ERROR') levelClass = 'text-rose-400 bg-rose-950/20 px-1 rounded border border-rose-900/20 font-bold';
    if (log.level === 'INFO') levelClass = 'text-indigo-400 bg-indigo-950/10 px-1 rounded border border-indigo-900/10';
...
function renderResourceChart() {
  ...
  historicalData.forEach((pt, idx) => {
    ...
    if (idx % 3 !== 0 && idx !== historicalData.length - 1) return;
    const x = paddingLeft + idx * stepX;
  ...
  setupChartHover('resource-chart-container', width, paddingLeft, stepX, (idx, cursorX) => {
    ...
    if (hoverValL) hoverValL.setAttribute('style', 'display: none;');
    if (dotC) dotC.setAttribute('style', 'display: none;');
    if (dotM) dotM.setAttribute('style', 'display: none;');
...
function renderTrafficChart() {
  ...
  setupChartHover('traffic-chart-container', width, paddingLeft, stepX, (idx, cursorX) => {
    ...
    if (hoverValL) hoverValL.setAttribute('style', 'display: none;');
    if (dotR) dotR.setAttribute('style', 'display: none;');
    if (dotC) dotC.setAttribute('style', 'display: none;');
    if (tooltip) tooltip.classList.add('hidden');
  });
}
...
async function runPipeline() {
  ...
  // Toggle buttons
  const triggerBtn = $('trigger-build-btn') | null;
  ...
  const shouldFail = ($('inject-build-fail-checkbox')).checked || false;
  const failAtIdx = parseInt(($('fail-step-selector')).value || '0', 10);
  ...
  for (let i = 0; i < pipelineSteps.length; i++) {
    ...
    if (shouldFail && i === failAtIdx) {
      ...
      addIncident(
        'edge-cdn',
        'critical',
        `CI/CD Deployment failed on stage: "${pipelineSteps[i].name}". Environment targeting: ${pipelineTargetEnv}.`
      );
...
function updatePipelineStepUI(idx) {
  ...
  if (step.status === 'pending') {
    ...
    if (stepDur) stepDur.classList.add('hidden');
    if (stepErr) stepErr.classList.add('hidden');
...
function addPipelineConsoleLog(line, level) {
  ...
  if (level === 'ERROR') colorClass = 'text-rose-400 font-bold';
...
function initEventListeners() {
  ...
  toggleSpikeBtn?.addEventListener('click', () => {
    ...
    if (isTrafficSpike) {
      ...
      if (spikeIcon) spikeIcon.classList.add('text-rose-500');
      ...
      if (spikeIcon) spikeIcon.classList.remove('text-rose-500');
  ...
  toggleDegradeBtn?.addEventListener('click', () => {
    ...
    if (isCdnDegraded) {
      ...
      if (degradeIcon) degradeIcon.classList.add('text-amber-500');
      ...
      if (degradeIcon) degradeIcon.classList.remove('text-amber-500');
  ...
  const slider = $('cpu-threshold-slider')  | null;
  slider?.addEventListener('input', () => {
    cpuAlertThreshold = parseInt(slider.value, 10);
    const label = $('cpu-threshold-label');
    if (label) label.textContent = `${cpuAlertThreshold}%`;
  });
  ...
  toggleConfigBtn?.addEventListener('click', () => {
    configContentArea?.classList.toggle('hidden');
    configChevron?.classList.toggle('rotate-180');
  });
  ...
  $('trigger-build-btn')?.addEventListener('click', () => {
    runPipeline();
  });

  // 10. Reset Pipeline Build
  $('reset-build-btn')?.addEventListener('click', () => {
    resetPipeline();
  });

  // 11. Fail checkbox conditional dropdown
  const failCheckbox = $('inject-build-fail-checkbox')  | null;
  const failStepSelector = $('fail-step-selector');
  failCheckbox?.addEventListener('change', () => {
    if (failCheckbox.checked) {
      failStepSelector?.classList.remove('hidden');
    } else {
      failStepSelector?.classList.add('hidden');
    }
  });
  ...
  logsLevelSelector?.addEventListener('change', () => {
    renderLogs();
  });

  const logsServiceSelector = $('logs-service-selector');
  logsServiceSelector?.addEventListener('change', () => {
    renderLogs();
  });
